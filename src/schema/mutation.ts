...
import { generateResponse, isValidEmail, isValidName, isValidUsername, logError, signInProviders } from '../utils/index.js';
...
export const PostMutations = extendType({
  ...
  definition(t) {
    ...
    t.field('createUser', {
      ...
      async resolve(_, { email, provider, verificationStatus, name, username, profilePictureMediaId, signUpIpv4Address }, { dataSources, req }) {
        try {
          if (!email || !provider || !verificationStatus || !name || !username) {
            return generateResponse(true, 'Something went wrong while validating your request', 'inputParamsValidationFailed', 403, null);
          }
          if (!isValidUsername(username)) {
            return generateResponse(true, 'Username must be between 8 and 15 characters long', 'usernameValidationFailed', 403, null);
          }
          if (!isValidName(name)) {
          ...
          const response = await dataSources
            .UserAPI()
            .createUser({ email, provider, verificationStatus, name, username, profilePictureMediaId, signUpIpv4Address });
          return response;
        } catch (error) {
