  ...
  definition(t) {
    ...
    t.field('createUser', {
      ...
      args: {
        ...
        headerPictureMediaId: nullable(stringArg()),
        signUpIpv4Address: nullable(stringArg()),
      },
      async resolve(_, { userIdentifier, provider, name, username, profilePictureMediaId, headerPictureMediaId, signUpIpv4Address }, { dataSources, req }) {
        const token = req.headers.authorization;
        ...
        try {
          ...
          const response = await dataSources
            .UserAPI()
            .createUser({ userId, email, provider, name, username, profilePictureMediaId, headerPictureMediaId, signUpIpv4Address });
          return response;
        } catch (error) {
    ...
    t.field('updateUser', {
      ...
      args: {
        ...
        website: nullable(stringArg()),
        socialLinks: nullable(list(nonNull(stringArg()))),
      },
      async resolve(_, { userId, name, profilePictureMediaId, headerPictureMediaId, bio, location, website, socialLinks }, { dataSources, req }) {
        const token = req.headers.authorization;
        const user = await AuthUtil().verifyToken(token);
        try {
          if (!userId || !name) {
          ...
          const response = await dataSources
            .UserAPI()
            .updateUser({ userId, name, profilePictureMediaId, headerPictureMediaId, bio, location, website, socialLinks });
          return response;
        } catch (error) {
          logError(error.message, 'updateUserError', 5, error, { args: req.body?.variables });
          return generateResponse(true, `Something went wrong while updating user. We're working on it`, 'updateUserError', 500, null);
        }
      },
    });
  },
